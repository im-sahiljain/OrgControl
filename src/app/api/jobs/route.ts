...
export async function PUT(req: Request) {
  try {
    ...
    await dbConnect();
    const body = await req.json();
    const id = body.jobId || body.id || body._id;
    const updatePayload = body.updatedFields || body;
    ...
    const { title, department, location, type, description, requirements, status } = updatePayload;
