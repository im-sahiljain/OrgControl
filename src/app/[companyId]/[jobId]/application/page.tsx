...
import { Building2, MapPin, Clock, ArrowLeft, Loader2, Upload, FileText, CheckCircle2, AlertCircle } from "lucide-react";
...
export default function JobDetailPage() {
  ...
  // Cloudinary structured upload mutation
  const uploadResumeMutation = useMutation({
    ...
    onError: (err: any) => {
      console.error(err);
      setErrorMsg("Failed to upload resume PDF to Cloudinary directory.");
      setUploading(false);
    },
  });
  ...
  // Submit and Parse Candidate details
  const applyMutation = useMutation({
    mutationFn: async () => {
      ...
      let aiInsights = {
        matchScore: 0,
        skills: [] as string[],
        summary: "Pending AI screening. Use the Recruitment Workspace to trigger AI review.",
        pros: [] as string[],
        cons: [] as string[],
      ...
      try {
        // 1. Run AI screener parser route
        ...
        console.warn("AI screening route failed during submit. Saving candidate as unscreened.", err);
      }

      // 2. Save the candidate record in database
    ...
    onError: (err: any) => {
      console.error(err);
      setErrorMsg(err.response?.data?.error || "An error occurred while analyzing your resume application.");
      setAiParsing(false);
    },
  });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!name || !email || !phone || !resumeUrl) {
      alert("Please fill all details and upload your resume PDF.");
      return;
    }
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950 flex flex-col items-center justify-center space-y-4">
        ...
        <p className="text-sm text-zinc-505 font-medium">Loading position specifications...</p>
      </div>
    );
  }
  ...
  if (!job) {
    return (
      <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950 flex flex-col items-center justify-center space-y-4">
        ...
        <h3 className="text-lg font-bold text-zinc-900 dark:text-zinc-50">Position Not Found</h3>
  ...
  return (
    <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950 flex flex-col font-sans">
      <header className="sticky top-0 z-10 bg-white/80 dark:bg-zinc-900/80 backdrop-blur-md border-b border-zinc-200 dark:border-zinc-800 px-6 h-16 flex items-center justify-between">
        <Link href={`/${companyId}/jobs`} className="flex items-center gap-2 text-sm text-zinc-500 hover:text-zinc-850 dark:hover:text-zinc-100 font-bold transition-colors">
      ...
      <main className="flex-1 max-w-4xl w-full mx-auto p-6 grid grid-cols-1 md:grid-cols-5 gap-8 items-start">
        ...
        <div className="md:col-span-3 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-6 shadow-sm space-y-6">
          <div className="space-y-3">
            ...
            <h1 className="text-2xl font-bold text-zinc-900 dark:text-zinc-55">{job.title}</h1>
            <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-xs text-zinc-505">
              <span className="flex items-center gap-1"><MapPin className="h-4 w-4 shrink-0" /> {job.location}</span>
              <span className="flex items-center gap-1"><Clock className="h-4 w-4 shrink-0" /> {job.type}</span>
            </div>
          </div>

          <div className="border-t border-zinc-100 pt-6 space-y-4">
            <h3 className="font-bold text-sm text-zinc-800 dark:text-zinc-200 uppercase tracking-wider">Position Description</h3>
            <p className="text-sm text-zinc-650 dark:text-zinc-400 leading-relaxed whitespace-pre-wrap">{job.description}</p>
          </div>

          {job.requirements && job.requirements.length > 0 && (
            <div className="border-t border-zinc-100 dark:border-zinc-800 pt-6 space-y-4">
              <h3 className="font-bold text-sm text-zinc-800 dark:text-zinc-200 uppercase tracking-wider">Requirements & Qualifications</h3>
        ...
        <div className="md:col-span-2 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-6 shadow-sm">
          {submitted ? (
            <div className="text-center py-10 space-y-4 animate-fade-in">
              ...
              <h3 className="text-xl font-bold text-zinc-900 dark:text-zinc-50">Application Submitted!</h3>
              <p className="text-xs text-zinc-500 leading-relaxed">
                Thank you for applying. Your resume has been uploaded to the job's Cloudinary records and analyzed by our AI recruiter pipeline. Our HR team will review your insights shortly.
              </p>
              <Link href={`/${companyId}/jobs`} className="block pt-2">
                <Button className="w-full bg-blue-600 hover:bg-blue-700 text-white">Back to Jobs Portal</Button>
              </Link>
            </div>
          ) : (
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {/* Resume File Upload */}
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-zinc-500">Resume PDF</label>
                  ...
                  {resumeUrl ? (
                    <div className="p-3 border border-zinc-200 dark:border-zinc-800 rounded-lg bg-zinc-50/50 dark:bg-zinc-950/50 flex items-center justify-between gap-3 text-xs">
                      <div className="flex items-center gap-2 truncate">
                        ...
                        <span className="font-bold text-zinc-700 dark:text-zinc-300 truncate">{fileName || "Uploaded Resume"}</span>
                      </div>
                      <button
                        type="button"
                        onClick={() => { setResumeUrl(""); setFileName(""); }}
                    ...
                    <button
                      ...
                      className="p-6 border-2 border-dashed border-zinc-200 dark:border-zinc-800 hover:border-blue-500 rounded-xl flex flex-col items-center justify-center gap-1.5 text-xs text-zinc-550 bg-zinc-50/20 transition-all w-full"
                    ...
                    >
                      ...
                      <span className="font-bold">{uploading ? "Uploading PDF..." : "Upload Resume PDF"}</span>
                      <span className="text-[10px] text-zinc-400">Only PDF files are supported.</span>
                    </button>
                  )}
                </div>
              </div>
